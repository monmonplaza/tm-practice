...
class UserOther
{
    ...
    public function __construct($db)
    {
        ...
        $this->tblEmployee = "hrisv2_employee";
        $this->tblJobTitle = "hrisv2_settings_job_title";
        $this->tblLeaveType = "hrisv2_settings_leavetype";
    }
    ...
    public function readAll()
    {
        try {
            ...
            $sql .= "employee.employee_job_email, ";
            ...
            $sql .= "order by user.user_other_is_active, ";
            $sql .= "role.role_name desc, ";
            $sql .= "employee.employee_lname, ";
            $sql .= "employee.employee_fname asc ";
    ...
    public function readLogin()
    {
        try {
            ...
            $sql .= "role.*, ";
            $sql .= "title.job_title_name, ";
            $sql .= "leaveType.leavetype_is_default, ";
            $sql .= "employee.employee_aid, ";
            $sql .= "employee.employee_gender, ";
            $sql .= "employee.employee_job_supervisor_id, ";
            $sql .= "employee.employee_job_leave_type_id, ";
            $sql .= "employee.employee_lname, ";
            $sql .= "employee.employee_fname, ";
            $sql .= "employee.employee_photo, ";
            $sql .= "employee.employee_job_email ";
            $sql .= "from {$this->tblUserOther} as user, ";
            $sql .= "{$this->tblJobTitle} as title, ";
            $sql .= "{$this->tblRole} as role, ";
            $sql .= "{$this->tblEmployee} as employee, ";
            $sql .= "{$this->tblLeaveType} as leaveType ";
            $sql .= "where user.user_other_role_id = role.role_aid ";
            $sql .= "and user.user_other_emp_id = employee.employee_aid ";
            $sql .= "and employee.employee_job_leave_type_id = leaveType.leavetype_aid ";
            $sql .= "and employee.employee_job_title_id = title.job_title_aid ";
            $sql .= "and employee.employee_job_email like :employee_job_email ";
            $sql .= "and user.user_other_is_active = 1 ";
            $query = $this->connection->prepare($sql);
            $query->execute([
                "employee_job_email" => $this->employee_job_email,
            ]);
        } catch (PDOException $ex) {
            $query = false;
        }
    ...
    public function search()
    {
        try {
            ...
            $sql .= "or concat(employee.employee_fname, ' ' , employee.employee_lname) like :fullname ";
            $sql .= ") ";
            $sql .= "order by user.user_other_is_active, ";
            $sql .= "role.role_name desc, ";
            $sql .= "employee.employee_lname, ";
    ...
    public function readById()
    {
        try {
            ...
            $sql .= "order by user_other_emp_id asc ";
            $query = $this->connection->prepare($sql);
            $query->execute([
    ...
    // reset password for settings user other
