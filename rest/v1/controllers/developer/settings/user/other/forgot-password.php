...
if (isset($_SERVER['HTTP_AUTHORIZATION'])) {
    ...
    $user_other->employee_job_email = trim($data["email"]);

    $password_link = "/other/create-password";

    $query = $user_other->readLogin();
    $subQuery = $query->fetchAll();
    ...
    $user_other->user_other_emp_id = $subQuery[0]["employee_aid"];

    $query = checkForgotPassword($user_other);
    $mail = sendEmail(
        $password_link,
        $user_other->employee_job_email,
        $user_other->user_other_key
    );
    http_response_code(200);
    $returnData["data"] = [];
    $returnData["count"] = $user_other->employee_job_email;
