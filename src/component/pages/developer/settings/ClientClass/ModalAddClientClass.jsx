...
const ModalAddClientClass = ({itemEdit}) => {
  ...
  const mutation = useMutation({
    mutationFn: (values) =>
      queryData(
        itemEdit
          ? `/v1/engagement-category/${itemEdit.engagement_category_aid }`
          : "/v1/engagement-category",
        itemEdit ? "put" : "post",
        values
      ),
    onSuccess: (data) => {
      // Invalidate and refetch
      queryClient.invalidateQueries({ queryKey: ["engagementcategory"] });
...
//initial value of the form
  const initVal = {
    ...
    engagement_category_description: itemEdit ? itemEdit.engagement_category_description : "",
    engagement_category_invoice_description: itemEdit ? itemEdit.engagement_category_invoice_description : "",
  };

//validation of required inputs
  const yupSchema = Yup.object({
    engagement_category_name: Yup.string().required("Required"),
